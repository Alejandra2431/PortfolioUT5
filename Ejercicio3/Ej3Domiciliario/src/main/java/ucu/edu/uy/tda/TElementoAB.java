...
public class TElementoAB<T> implements IElementoAB<T> {
    ...
    public TElementoAB(Comparable unaEtiqueta,int frec , T unosDatos) {
        etiqueta = unaEtiqueta;
        datos = unosDatos;
        frecExito = frec;
        frecNoExitoDer = 0;
...
// tiene los dos hijos, buscamos el lexicograficamente anterior
    ...
    public int obtenerCosto(int contNivel){
        int contador = 0;
        if (hijoIzq != null) {
            contador += this.hijoIzq.obtenerCosto(contNivel++);
        }        
        if (hijoDer != null) {
            contador += this.hijoDer.obtenerCosto(contNivel++);
        }
        return  this.frecExito * contNivel + contador;
    }
